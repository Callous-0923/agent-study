...
📌 本章目标：
  ...
  3. 理解 Heartbeat 与后台记忆整理这两类历史/工程模式
  4. 掌握记忆写入、检索、淘汰和评测的完整生命周期
  5. 了解当前 Letta 的 memory blocks / files / archival memory / external RAG
...
16.4 后台记忆整理（Sleep-Time Compute 模式）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

这是一种后台整理模式，不应把特定产品版本或调度细节当作通用协议。
...
效果必须在包含长程召回、时间冲突、隐私删除和错误记忆修正的测试集上测量；
单个公开 benchmark 分数不能证明某种存储方案对所有业务更好。


16.5 当前 Letta 的上下文层次 —— 按用途选存储
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

当前 Letta 文档区分四类上下文来源：
  - Memory Blocks：始终在上下文内，适合少量高价值、可编辑信息
  - Files：按需打开/搜索的只读内容，适合文档与项目材料
  - Archival Memory：上下文外、可写可检索的记忆片段
  - External RAG：通过自定义工具或 MCP 连接的大规模外部数据
...
核心启示：Agent 的记忆质量取决于写入策略、检索策略、冲突处理、遗忘/删除、
权限与评测；不存在对所有规模和数据类型都最优的单一存储抽象。
...
▍ Heartbeat 的成本计算 —— 「自我纠错」要花多少钱？
  ...
  每次 Heartbeat 触发时，Agent 要把「当前状态 + 工具结果」发给 LLM，
  让 LLM 判断是否需要进一步行动。这确实增加了成本。

  控制策略：
    a) 选择性 Heartbeat —— 不是每个工具都请求心跳，
       只在「关键工具」（数据库写入、API 调用）执行后触发
    b) 对整理任务做模型路由 —— 只有评测证明小模型达标时才下沉
    c) Heartbeat 超时限制 —— 最多触发 3 次心跳，避免死循环
    d) Token 预算驱动 —— 累计消耗超过阈值 → 停止心跳，直接总结

  成本、错误率和收益与触发频率及任务分布有关，必须通过对照实验报告。
...
▍ Sleep-Time Compute 的调度设计
  ...
  工程实现建议：
    - 用户交互结束后按产品延迟目标排队，而不是固定等待某个秒数
    - 新交互到来时合并、取消或重新排队，避免并发覆盖
    - Worker 扫描增量对话并用幂等键提交候选记忆
    - Sleep-Time Agent 扫描本轮对话 → 提取要点 → 更新 Core Memory
    - 完成后 Agent 回到待机状态

  注意：Sleep-Time Compute 不是实时需求 → 可以异步执行，
  不阻塞用户的下一次交互。这也是为什么 Letta 使用后台 Agent 的原因。
...
▍ 记忆压缩的工程权衡 —— 保留什么，丢弃什么？
  ...
  面试可以主动分析：
    值得保留 → 用户偏好（「我喜欢简洁回答」）、事实信息（「我是北京人」）、
              任务上下文（「正在做税务报表，已完成 Q1-Q3」）
    可以丢弃 → 临时搜索词、失败的尝试、已过时的数据、
              与当前任务无关的闲聊内容

  工业界的经验法则：Core Memory 控制在 2000 tokens 以内
  （留给对话历史足够的空间）。超过上限 → 触发记忆整理
  （合并相似、删除过时、归档低频）
...
4. Letta 当前上下文层次的启示
   - Memory Blocks / Files / Archival Memory / External RAG 各有边界
   - 文件操作可审计且易搜索，但不自动解决冲突、权限和删除
   - 选型必须匹配数据规模、重要性、读写方式与评测结果
