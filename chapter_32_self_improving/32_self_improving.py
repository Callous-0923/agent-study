...
生产闭环还必须增加：数据去重/隐私处理 → 候选版本 → 冻结回归集 → 安全审查
→ 离线门槛 → 小流量灰度 → 线上守护指标 → 晋升或自动回滚。
