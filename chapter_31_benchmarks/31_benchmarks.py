...
如果你面试时被问「你们怎么评测 Agent」，GAIA 是你第一个该提的名字。
它是 Meta 在 2023 年发布的，目前 Agent 评测领域的「SAT 考试」。
...
与使用静态页面或沙箱的 Web 评测相比，ClawBench 的关键设计包括：
  ...
  - 安全提交拦截：记录并阻断最终提交请求，避免产生真实副作用
  - 端到端证据：保存动作、HTTP 请求、截图、会话消息和录像，支持结果审计

规模：论文介绍了 153 个日常任务，覆盖 144 个网站和 15 个生活与工作类别。
...
if __name__ == "__main__":
    ...
    print("║  GAIA · AgentBench · WebArena · ClawBench · tau-bench║")
    print("╚══════════════════════════════════════════════════════╝")
    demo_benchmarks()
    print("\n▶ 六大评测对比")
    print("-" * 50)
