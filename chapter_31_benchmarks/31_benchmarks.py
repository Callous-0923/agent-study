...
📌 本章目标：
  1. 掌握 GAIA / AgentBench / WebArena / ClawBench / tau-bench / SWE-bench 六大评测
  2. 理解每个评测的设计意图和适用场景
  3. 学会自建 Agent 评测集的方法
  4. 建立生产环境的多维度评测体系
...
│ ClawBench      │ 日常在线任务  │ 生产网站+提交拦截         │
│ tau-bench      │ 工具使用精度  │ 函数参数正确率            │
│ SWE-bench      │ 代码修复     │ 真实 GitHub Issue        │
└────────────────┴──────────────┴──────────────────────┘

各评测的任务、环境与成功标准不同，不使用统一星级比较难度。
...
31.5 ClawBench —— 真实生产网站上的日常任务评测
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

评测什么：衡量浏览器 Agent 能否在真实生产网站上完成日常在线工作流，
包括跨页面搜索、多步表单、预约、购物和求职等操作。

与使用静态页面或沙箱的 Web 评测相比，ClawBench 的关键设计包括：
  - 真实生产网站：保留动态内容、布局变化和实际交互复杂性
  - 提交拦截：按任务规则记录并阻断关键、不可逆的最终 HTTP 请求，
    如结账、表单提交或发送邮件，防止这些请求产生真实副作用。
    该机制依赖请求规则覆盖，不能视为对所有网站操作的安全保证；
    拦截到请求也不等于任务成功，仍需后续评估。
  - 端到端证据：保存动作、HTTP 请求、截图、会话消息和录像，支持结果审计

规模：论文 v2（2026-07-20）介绍了 153 个日常任务，覆盖 144 个平台和 15 类。
版本说明（2026-09-05 核对）：官方仓库当前 V1 为 152 个任务、143 个网站；
这是发布后移除任务的开源语料口径，不能与论文口径混用。
这类评测关注复杂流程的最终任务完成率，而不只是单步点击准确率。
报告结果时记录任务集版本、模型、harness、预算与运行日期；网站布局、
可用性和反自动化机制的变化也会影响结果。

论文：https://arxiv.org/abs/2604.08523v2
代码：https://github.com/TIGER-AI-Lab/ClawBench
