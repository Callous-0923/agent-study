...
但 Agent 系统的故障模式完全不同，更复杂、更危险。因为 Agent
不是「一个请求走完就结束」的模式——它是多轮迭代的思考过程。

Agent 特有的 4 大故障模式（面试必问）：
  1. LLM 返回幻觉参数 → 工具执行报错
     例如 LLM 把订单号 "ORD-123" 理解成 "ORD_123"，工具找不到这个订单。
     这在传统 API 中不存在——传统 API 的参数都是程序传入的，不会「猜错」。

  2. Agent 陷入死循环 → 无限消耗 Token
     例如 LLM 反复调用 search_weather → 每次都说「没找到」→ 再调一遍。
     一个死循环可能几分钟烧掉 $10 的 API 费用。传统 API 不存在「自己调用自己」的循环。

  3. 工具调用雪崩 → 一个失败导致后续全部失败
     Agent 先调 search_db，超时了。LLM 拿到超时结果，又调了另一个搜索工具。
     此时上下文被大量失败信息污染，后续判断全偏了。

  4. 上下文爆炸 → 对话历史过长 → LLM 行为异常
     经过 20 轮工具调用后，messages 数组里堆满了 tool_call 和 tool_result。
     超过 LLM 上下文窗口后，Agent 可能丢失关键信息。

可靠性的核心定义：系统在故障下仍能提供「可接受」的服务。
对于 Agent 来说，可接受意味着：不进入死循环、不重复扣费、不丢失关键信息。
