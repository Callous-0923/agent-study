"""
第23章：代码 Agent 架构横评 —— 从产品榜单回到可验证设计
========================================================
...
产品与榜单变化很快；本章产品名只是 2026-08-01 的架构示例，不构成排名。
...
SWE-bench 是重要的代码修复 benchmark，但不是完整的软件工程能力度量。
...
2026 年的重要限制：
  - OpenAI 已停止把 SWE-bench Verified 作为前沿编码能力的主要指标，理由包括
    训练污染和部分测试缺陷
  - 2026-07 对 SWE-bench Pro 的审查也报告了大量有问题的任务
  - 公布分数只有在模型快照、harness、最大预算、测试镜像和日期一致时才可比
  - 项目选型应加入私有未污染 issue、代码审查、安全、测试质量和人工接管指标


23.4 Agentless 的颠覆性发现
━━━━━━━━━━━━━━━━━━━━━━━━━━

2024 年发表的 Agentless 工作展示了一条固定的定位→修复→验证流水线。
...
核心发现：
  ...
  和 Agent 的关键区别：
    ...
    ✗ 没有工具调用
    ✗ 没有自主决策
    → 在特定 benchmark 与预算下可以形成有竞争力的强基线

  为什么？

  1. 更多自主步骤会增加错误与方差
  2. 固定流程更容易复现、预算和消融
  3. 自主 Agent 只有在需要动态探索时才可能覆盖固定流程做不到的分支
...
2. SWE-bench 是重要外部参照，但 Verified 与 Pro 都暴露出污染/任务质量问题
   必须同时维护私有回归集，并报告 harness、预算、镜像和日期

3. Agentless 的启示
   简单方案常比复杂 Agent 更好
   固定流程应作为所有复杂 Agent 的强基线
...
if __name__ == "__main__":
    ...
    print("\n▶ SWE-bench 结果报告 Checklist")
    print("-" * 50)
    print("  模型快照 / harness commit / 容器镜像 / 最大预算")
    print("  运行日期 / 重试策略 / 失败分类 / 污染检查")
    print("  公开 benchmark + 私有未污染回归集")
