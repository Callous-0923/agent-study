...
产品功能、限额、许可证与价格变化很快；本章产品名是候选示例，所有选型结论
都应由同一数据集上的容量、召回、延迟、过滤和运维验证产生。
...
比较 Chroma、Pinecone、Milvus、Qdrant、pgvector 等候选时统一测量：
  - 数据规模、维度、写入速率、删除/更新频率与多租户隔离
  - 向量 + metadata filter + 关键词混合查询的 Recall@k / nDCG@k
  - 预热后 p50/p95/p99 延迟、吞吐、索引构建与恢复时间
  - 备份、可观测、升级、数据驻留、团队运维与总拥有成本

选型决策树（面试必备！）：

  你的需求是什么？
  ├─ 先做最小原型 → 本地/已有数据库扩展通常更省集成成本
  ├─ 不想管运维 → 比较多个托管服务的 SLO、数据驻留和退出成本
  ├─ 需要独立集群 → 用真实规模压测 Milvus / Qdrant 等候选
  └─ 已有 Postgres/Redis → 先评估 pgvector / Redis，避免过早新增系统
...
维度选择的权衡（面试高频）：
  高维度：
    可能保留更多模型信息，但索引更大、内存和距离计算更多
    
  低维度：
    存储和计算更小，但是否损失质量取决于模型、压缩方式和语料

  维度不是可跨模型比较的通用质量旋钮。对同一模型的候选维度，测端到端
  答案质量、Recall@k、索引体积和延迟，再选择满足质量门槛的最小值。

批量优化的工程技巧：
  单个请求发送 Embedding 是最大的浪费。
  按 provider 当前的单请求输入、token 与速率限制做批量，并为 429/5xx 重试；
  批量通常减少网络开销，但不会自动把 token 计费降到固定比例。
  │ Cohere Embed v3       │ 1024     │ 多语言    │ 企业级    │
  └──────────────────────┴──────────┴──────────┴──────────┘

维度对性能的影响必须在同一 embedding 模型和标注集上测量。下面的哈希向量
只演示存储与距离计算，不能产生真实 Recall 结论。
"""
...
接下来用一个教学实验直观展示维度如何影响存储和计算。这里的“模拟分数”
来自哈希向量，不代表真实 Embedding 质量，也不能推出某个维度区间普遍最优。
生产选型必须固定同一模型、同一数据集和索引参数，比较 Recall@k、端到端答案
质量、索引体积、构建时间和 p95 查询延迟。
"""
...
def demo_embedding_dimension_tradeoff():
    ...
    print(f"\n  {'维度':<8s} {'检索时间':<12s} {'存储(字节)':<12s} {'模拟分数':<10s}")
    print(f"  {'-'*42}")
    ...
    for dim in [128, 256, 384, 768, 1536]:
        ...
        print(f"  {dim:<8d} {elapsed*1000:<12.2f}ms {storage:<12d} {score:.3f}")
...
3. 维度选择
   用同一模型和标注集选择满足质量门槛的最小维度

4. 混合检索
   纯向量搜索不够 → 加关键词匹配（BM25）
   是否启用混合检索由语料、查询类型和离线评测决定
...
4. 混合检索是重要候选，但不是无需评测的固定答案

面试速记：
  「向量数据库怎么选？」
  → 用规模、过滤、召回、延迟、恢复和总成本比较候选
  → 维度只在同一 embedding 模型内做实测权衡
  → 向量、关键词、重排的组合必须通过消融评测
"""
...
if __name__ == "__main__":
    ...
    print("-" * 50)
    for item in [
        "本地原型 → 比较 Chroma 与现有数据库扩展",
        "托管优先 → 比较 SLO、数据驻留与退出成本",
        "独立集群 → 用真实规模压测 Milvus/Qdrant 等候选",
        "已有 PostgreSQL → 先验证 pgvector 是否已满足需求",
    ]:
        print(f"  {item}")
    print("\n✅ 第25章完成！")
